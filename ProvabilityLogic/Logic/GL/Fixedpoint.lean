...
/-!
# Fixed point theorem for GL

A formula `A` in which the atom `p` is modalized (`Formula.ModalizedIn`) has a fixed point,
unique up to GL-provable equivalence: `LogicGL.fixpointTheorem`.
...
theorem remove_modalized_atom_ant
  ...
  have hM' : ∀ C : Formula α, C.ModalizedIn p →
      (x ⊩[M'] C ↔ x ⊩[M] C) :=
    fun C hC => forces_iff_of_modalized C hC;
  obtain ⟨D, hD, hfD⟩ := Kripke.finite_soundness h M' x (by
    intro C hC;
    rcases Finset.mem_insert.mp hC with rfl | hC;
    . exact val_self.mpr trivial;
...
theorem fixpoint_uniqueness (hA : A.ModalizedIn p) :
  ...
  have hval := val_iff_of_fixpoints (x := x) (q := q) hA
    (by
      intro y hy;
      rcases hy with rfl | hy;
      . have := forces_boxdot.mp h₁ |>.1; grind;
      . have := forces_boxdot.mp h₁ |>.2 y hy; grind)
    (by
      intro y hy;
      rcases hy with rfl | hy;
      . have := forces_boxdot.mp h₂ |>.1; grind;
...
/-- Uniqueness for arbitrary formulas `D` and `E` in place of the atoms `p` and `q`.
...
/-- The fixed point of `A`, extracted as a Maehara interpolant. -/
...
theorem fixpoint_existence (hpq : p ≠ q) (hA : A.ModalizedIn p) (hq : q ∉ A.atoms) :
  ...
  have h₂ : ⊢ᵍ[GL] ((insert D {□((A⟦p ↦ #q⟧) 🡘 #q)}) ⟹
    ...
    rw [hD'] at this;
    simpa [fixpointPartition] using this;
  have h₄ : ⊢ᵍ[GL] (({A, □(A 🡘 #p)} : FormulaFinset α) ⟹ {D}) :=
    remove_modalized_atom_ant (p := p)
      (by
        intro C hC;
        rcases Finset.mem_insert.mp hC with rfl | hC;
        . exact hA;
      ...
      h₁;
  have h₅ : ⊢ᵍ[GL] ((insert D {□((A⟦p ↦ #q⟧) 🡘 #q)}) ⟹ ({A⟦p ↦ #q⟧} : FormulaFinset α)) :=
    remove_modalized_atom_suc (p := q)
      (by
        intro C hC;
        rcases Finset.mem_insert.mp hC with rfl | hC;
...
/-- The fixed point theorem for GL. The fresh atom `q` serves only as a placeholder in the
