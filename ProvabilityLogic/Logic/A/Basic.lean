...
`LogicA` is `GL` extended by every instance `TBB n` of the scheme `□^[n + 1]⊥ 🡒 □^[n]⊥`,
equivalently by every iterated consistency statement `∼□^[n]⊥`. This file gives
substitution-free induction principles for `LogicA`, its deduction-theorem characterization
over `GL`, its ω-model completeness, and its proper inclusion in Dzhaparidze's `LogicD`.
...
consistency statements `∼□^[n]⊥` in place of the axioms `TBB n`. -/
...
/--
  ω-model completeness of `LogicA`.
