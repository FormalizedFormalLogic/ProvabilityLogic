...
/-!
# Semantic cut-elimination for `D`

Cut-free `LogicD.ProofGentzen`-provability of a level-`2` sequent coincides with
`LogicD.GentzenWithCutProof`-provability, and with validity at the root of every constant,
respectively free, ω-extension of every `GL`-model.

## References

- [KKIM25, Definition 5.2, Lemma 5.3, Theorem 5.8]
-/
...
/-- The cone of `t` in the finite `GL`-countermodel of `BS`, extended by a constant ω-chain
  sharing the valuation of the cone's root. -/
