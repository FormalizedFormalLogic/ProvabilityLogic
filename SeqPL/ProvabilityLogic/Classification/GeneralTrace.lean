...
/--
  ...
  forcing all axiom T instances `□B 🡒 B` for `B ∈ Γ` (the semantic core of the chain lemma).
...
lemma Model.exists_forces_axiomT_of_card_lt_rank [DecidableEq α] :
    ...
    ∃ z, x ≺ z ∧ ∀ B ∈ Γ, z ⊩ ((□B) 🡒 B) := by
  -- Induction on `Γ.card`: take a successor `z` of rank exactly `Γ.card`; if some
  -- `□B₀ 🡒 B₀` fails at `z` then `z ⊩ □B₀`, hence `□B₀ 🡒 B₀` holds automatically at
  -- every successor of `z`, and the induction hypothesis applies to `Γ.erase B₀`.
  intro n;
  induction n with
  | zero =>
    intro Γ hΓ x hx;
...
/--
  **Chain lemma** (instantiated to the boxed subformulas of `A`):
  `GL ⊢ ∼□^[m+1]⊥ 🡒 ◇⋀{□B 🡒 B | □B ∈ Sub(A)}` where `m` is the number of boxed
  subformulas. An actual proof of what Foundation assumes as the axiom
...
/--
  **Finiteness or cofiniteness of traces**: the trace of any formula is either finite or
  cofinite.
...
lemma Formula.trace_finite_or_cofinite [DecidableEq α] {A : Formula α} :
  ...
  -- If the trace is infinite, take a countermodel whose height exceeds the number of
  -- boxed subformulas; the chain lemma yields a world `a` forcing all axiom T instances,
  -- and `graft` then produces countermodels of every height `≥ M.height`.
...
/--
  If `L.trace` is cofinite then `L ⊆ LogicGLBetaMinus (L.trace)`.

- [AB05, Lemma 45 (second half)]
-/
...
/--
  ...
  Extracted from the proof of `subset_LogicS_addTBB_compl_trace_of_subset_LogicS`.

- [AB05, Lemma 49]
-/
lemma LogicGLBetaMinus.bot_mem_of_eq_univ {hCf : (Set.univ : Set ℕ)ᶜ.Finite} :
    (⊥ : Formula α) ∈ LogicGLBetaMinus Set.univ hCf := by
  -- Once `Betaᶜ`'s cofiniteness assumption is instantiated at `Beta = Set.univ`, the
  -- `TBBMinus` axiom ranges over the empty set and hence has empty spectrum, so it
  -- entails everything.
...
theorem provable_TBB_of_mem_trace {n : ℕ}
  ...
  -- Via the Solovay construction, applied to the countermodel `M`.
...
- [AB05, Lemma 49 (first half of proof)]
-/
...
/--
  ...
  proves the lifted `TBBMinus` axiom of its trace (the key step of the proof).
...
theorem provable_TBBMinus_of_not_subset_LogicS
  ...
  have hB : B ∈ L := by
    ...
    exact this.2;
  -- The Solovay sentences of `M₁.extendRoot 1`, used via the reflexive main lemma.
  let S := LO.FirstOrder.Theory.standardProvability.solovaySentences T (M₁.extendRoot 1);
...
/--
  If the provability logic `L` of `T` relative to `U` is not contained in `S`, then
  `L.trace` is cofinite and `L = LogicGLBetaMinus (L.trace)`.
