...
/-- `[Neg14]` Lemma 5.2, chain form: a looping sequent is provable. If there is a nonempty
...
/-- `ReflTransGen` variant of `loopChain`: the degenerate case `x = y` is closed by `axm`. -/
...
/-- `List`-chain form of `[Neg14]` Lemma 5.2: `x R y₁, …, yₙ₋₁ R yₙ` with
...
/-- `[Neg14]` Theorem 5.5, pigeonhole core: suppose that along a chain
...
repeats along the chain and the sequent is provable by Lemma 5.2 (`loopChain'`). -/
