...
/-- `subst` を経由しない `LogicA` の内在的定義（`LogicA.substlessInductionTBB` 用）． -/
...
/-- `LogicA` の帰納原理：`subst` を経由しない形（GL 部分・公理 `TBB n`・mdp）で帰納できる． -/
...
/--
  `LogicA` proves the iterated consistency statement `∼□^[n]⊥` for every `n`, by chaining
...
/--
  `subst` を経由しない `LogicA` の内在的定義（`substlessInductionGP` 用）．
  `LogicA` は `LogicGL` に `TBB n` ではなく `∼□^[n]⊥`（任意の `n`）を加えたものとしても
  特徴づけられる，というもう一つの公理化に対応する帰納法．
-/
...
/--
  `LogicA` のもう一つの帰納原理：`TBB n` ではなく `∼□^[n]⊥`（任意の `n`）を公理とみなして
  帰納できる．`LogicA` が `LogicGL` に `∼□^[n]⊥`（`n ∈ ℕ`）を加えたものでもある，という
  ことの反映．
-/
...
/--
  `LogicA` の定理は，任意の有限 GL モデルの任意の world において，
  `TBB 0, …, TBB (N-1)` が成立していれば成立する．pseudo-tail model や tail model・
  graftChainOmega のような frame の構成は不要で，Hilbert 導出を意味論的に並行して
  辿るだけで十分．
-/
...
/--
  world レベルでの成立から，任意の有限根付き GL モデルの根で `∼□^[N]⊥ 🡒 A` が成立する．
  `x ⊩ ∼□^[N]⊥ ↔ ∀ n < N, x ⊩ TBB n`（どちらも `x.rank ≥ N` と同値）という
  `Kripke.Rank` の事実だけで橋渡しできる．
-/
lemma root_forces_neg_boxItr_bot_imp
  (h : ∃ N : ℕ, ∀ {κ : Type u}, [Nonempty κ] → ∀ (M : Model κ α), [M.IsFiniteGL] → ∀ (x : M.World),
  (∀ n < N, x ⊩ TBB n) → x ⊩ A) :
...
/--
  **`LogicA`（Artemov のロジック `A`，`GLαω`）の deduction-theorem 形での GL による特徴づけ**．
  `LogicA.provability_TFAE`（`ProvabilityLogic/Classification/A_D.lean`）に至るまでの
  意味論的な核心部分：`graftChainOmega` や trace（`ProvabilityLogic/Classification/Trace.lean`）
  を一切経由せず，`Kripke.Rank` の rank の意味論のみから証明できる．
-/
...
/--
  **ω-model completeness of `GLαω`** (Lemma 5 in §3 of [Bek90], "On the classification
  ...
  used in the paper's proof. Unlike the `1 ↔ 2` equivalence above, this full
  characterization genuinely needs `graftChainOmega`: the `3 → 1` direction is proved via
  `exists_reflexive_countermodel_of_not_mem_LogicA`.
-/
