...
- **worktree での作業を終える際は，必ず以下の順序で実行する**．これは今後ユーザーの許可なく実行してよい．
  1. `lake build` で通ることを確認する．
  2. `just mk-all` を実行して `ProvabilityLogic.lean` を更新する（新しいファイルを追加した場合の import 漏れを防ぐため）．
  3. `just shake` を実行して未使用 import・不要な `public` を除去する．`lake shake` は 1. のビルドが完了していないと実行できないため，必ずビルド後に行う．
  4. shake が import を書き換えるため，もう一度 `lake build` を実行して通ることを確認する．3. の `--fix` が `native_decide` などの `meta` 定義に必要な `meta import` 行を，同じモジュールへの `public import` と重複していると誤認して削除し，ビルドを壊すことがある．**`meta import` 行を書く・見つけたときは，必ず行末に `-- shake: keep` アノテーションを付けて shake の削除対象から除外する**（例: `meta import ProvabilityLogic.Logic.GL.Basic -- shake: keep`）．4. のビルドが失敗した場合は，`Invalid \`meta\` definition ... is not accessible here; consider adding \`public meta import ...\`` のエラーを手がかりに，削除された `meta import` 行を復元し `-- shake: keep` を付けて再度ビルドを確認する．
  - **1つの論理的な作業を複数の worktree に分けて並列化している場合（上記の並列化ワークフローや，1つのリファクタリングを複数 worktree に分担させた場合など），この shake ステップは各 worktree で個別に実行する必要はない**．`lake shake` は全体ビルドが green であることを前提にしており，並列中の個々の worktree は他の worktree の変更や，無関係な既存の壊れ（他で修正中の既存バグ等）を含んでいないため全体ビルドが red のままになりがちで，個別に shake しても信頼できる結果にならない．各 worktree では `lake build` の確認までを行い，全ての worktree を統合先（`main` 等）へマージし終えた後，統合先で一度だけ `just mk-all` → `just shake` → `lake build` を実行すれば十分．
